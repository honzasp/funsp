...
jako o sérii změny \uv{časopozic}, kde časopozicí budeme nazývat dvojici
\emph{pozice} v hradu a \emph{času}, potřebného k cestě na tuto pozici modulo
perioda hradu (offset). V hradu je tedy celkem $w \times h \times p$ časopozic,
kde $w$ je šířka hradu, $h$ je výška a $p$ je perioda. Nejkratší cesta nikdy
...
Ve skutečnosti vždy provedeme řádově méně kroků než $whp$, jelikož v hradech bez
větších překážek nalezneme cestu rychle, v hradech s více překážkami sice cestu
nalezneme po delší době, ale velká část časopozic bude nedostupných.
...
počítat délky cest pomocí @t{length}, budeme používat typ @t{Path}, který
...
Funkce @t{moves} slouží k určení všech možných sousedních pozic, na které se
bílá paní může dostat z dané pozice. Tato funkce má typ @t{moves :: Bool ->
...
klasické pole @t{Array}, ale protože do tohoto pole budeme zapisovat po jednom
...
typu @t{IO} v tom, že umožňuje akcím využívat měnitelné datové struktury,
narozdíl od @t{IO} jsou však akce v monádě @t{ST s} omezeny pouze na
...
taková akce je \emph{deterministická} a monádu @t{ST s} tedy můžeme spustit a
získat její výsledek v čistém kódu.

Na akci typu @t{ST s a} je tedy možno pohlížet jako na výpočet, jenž využívá
interní stav a jehož výstup má typ @t{a}. Proměnná @t{s} slouží k zajištění, že
tento interní stav \uv{neunikne} z monády @t{ST}.  Funkce, která nám umožní
provést výpočet v monádě @t{ST} a získat jeho výstup se jmenuje @t{runST} a má
typ @t{(forall s. ST s a) -> a}.\footnote{Kvantifikátor @t{forall} je součástí
typové magie zajišťující bezpečnost použití @t{ST} a je součástí rozšíření
@t{ExistentialQuantification} jazyka Haskell.}
...
kde @t{s} je stejná typová proměnná, kterou předáme do @t{ST s}, @t{i} je typ
...
@t{newArray (a,b) x} vytvoří nové pole s rozsahem indexů od @t{a} do @t{b},
...
Nyní si implementujeme funkci @t{flood}, která pro daný seznam výchozích pozic s
...
kroku, v seznamu @t{rest} je zbytek, který zpracujeme v dalších krocích.

Následně pro každou dvojici ze seznamu @t{starts} zkontrolujeme, jestli v poli
@t{bests} není náhodou již nalezena nějaká jiná cesta, která je tedy lepší.
Pokud ne, do pole zapíšeme tuto cestu a vrátíme jak tuto dvojici, tak seznam
sousedních políček, \uv{zabalené} v konstruktoru @t{Just}. Pokud již byla
nalezena lepší cesta, vrátíme @t{Nothing}.
...
Veškerou práci ve funkci @t{navigate} provedeme v monádě @t{ST s}, kterou
pomocí @t{runST} \uv{spustíme} a získáme výsledek. Poté, co zjistíme šířku a
...
těchto pozic projdeme a pomocí funkce @t{moves} (jíž jako první argument
předáme @t{True}) získáme seznam pozic, do kterých se z nich můžeme jedním
pohybem i skrz zeď dostat. Proměnná @t{nexts} tedy obsahuje seznam pozic a cest,
které můžou mít o jednu zeď v cestě více. S tímto seznamem poté znovu rekurzivně
zavoláme @t{wallStep}, která nyní najde i cesty za právě prošlými zdmi.
